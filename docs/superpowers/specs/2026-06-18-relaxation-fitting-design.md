...
- Plot (`plot=True`): data + fitted curve, parameter textbox (value plus
  ...
  residuals panel below, the same two-panel layout as `_plot_fit_results`.
...
Logs a per-model R²/chi-squared comparison and the best model by **lowest
reduced chi-squared** (not R²): R² is biased toward models with more free
parameters (confirmed empirically: noisy data generated by
`mono_exponential` was mis-ranked as `biexponential` under an R² comparison,
since the extra parameters fit noise better). Reduced chi-squared already
accounts for degrees of freedom and is the correct metric for comparing
models of different complexity. If `plot=True`, draws all successful fits
overlaid on the data with a residuals panel, mirroring
`fit_multiple_shapes`/`_plot_comparison`.
