...
try:
    ...
    print(
        "Error: Could not import 'eprload'. Make sure eprload.py is in the same directory or accessible in the Python path.",
        file=sys.stderr,
    )
    sys.exit(1)
...
BRUKER_PARAM_MAP = {
    ...
    "XMIN": {
        "fair_name": "x_axis_minimum",
        "unit": "G",
        "description": "Minimum value of the X-axis (unit defined by XUNI).",
    },
    "XWID": {
        "fair_name": "x_axis_width",
        "unit": "G",
        "description": "Width (range) of the X-axis (unit defined by XUNI).",
    },
    "YMIN": {
        "fair_name": "y_axis_minimum",
        "unit": "MHz",
        "description": "Minimum value of the Y-axis (unit defined by YUNI).",
    },  # Assuming Y is frequency here
    "YWID": {
        "fair_name": "y_axis_width",
        "unit": "MHz",
        "description": "Width (range) of the Y-axis (unit defined by YUNI).",
    },  # Assuming Y is frequency here
...
def _process_parameters(pars: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    for key, value in pars.items():
        ...
        if key in BRUKER_PARAM_MAP:
            ...
            print(
                f"Parameter '{key}' not found in BRUKER_PARAM_MAP. Storing in 'unmapped_parameters'."
            )
...
def _save_to_csv_json(
    ...
    try:
        with open(json_file, "w", encoding="utf-8") as f:
            json.dump(
                metadata_to_save, f, indent=4, default=str
            )  # default=str handles non-serializables
    except IOError as e:
        warnings.warn(f"Could not write JSON file {json_file}: {e}")
    except TypeError as e:
        warnings.warn(
            f"Error serializing metadata to JSON for {json_file}: {e}. Some parameters might not be saved correctly."
        )
    ...
    try:
        if value is None:
            ...
            try:
                h5_object.attrs[
                    key
                ] = value  # Let h5py handle if possible (newer versions are better)
            except TypeError:
        ...
        warnings.warn(
            f"Could not store attribute '{key}' (type: {type(value)}) directly in HDF5 attributes. Converting to string."
        )
        h5_object.attrs[key] = str(value)
    except Exception as e:
        warnings.warn(
...
def _save_to_hdf5(
    ...
    try:
        with h5py.File(h5_file, "w") as f:
            ...
            f.attrs[
                "converter_script_version"
            ] = "fair_converter_v1.1"  # Example version
            ...
            if x is None:
                ...
                _try_set_h5_attr(
                    ds_x,
                    "description",
                    f"X axis ({fair_meta.get('axis_x_unit',{}).get('description','Unknown')})",
                )
                ...
                if len(x) >= 2 and x[1] is not None and isinstance(x[1], np.ndarray):
                    ...
                    _try_set_h5_attr(
                        ds_y_ax,
                        "description",
                        f"Y axis ({fair_meta.get('axis_y_unit',{}).get('description','Unknown')})",
                    )
                ...
                if len(x) >= 3 and x[2] is not None and isinstance(x[2], np.ndarray):
                    ...
                    _try_set_h5_attr(
                        ds_z_ax,
                        "description",
                        f"Z axis ({fair_meta.get('axis_z_unit',{}).get('description','Unknown')})",
                    )
            ...
            if "intensity" in data_grp:
                ...
                if current_ndim >= 1 and "x" in axis_datasets:
                    ...
                    try:
                        ...
                        warnings.warn(
                            f"OverflowError setting label/scale for X dim (index {x_dim_index}): {e_ovfl}. Skipping dimension link."
                        )
                    except Exception as e:
                        warnings.warn(
                            f"Error setting label/scale for X dim (index {x_dim_index}): {type(e).__name__} - {e}. Skipping dimension link."
                        )
                ...
                if current_ndim >= 2 and "y" in axis_datasets:
                    ...
                    try:
                        ...
                        warnings.warn(
                            f"OverflowError setting label/scale for Y dim (index {y_dim_index}): {e_ovfl}. Skipping dimension link."
                        )
                    except Exception as e:
                        warnings.warn(
                            f"Error setting label/scale for Y dim (index {y_dim_index}): {type(e).__name__} - {e}. Skipping dimension link."
                        )
                ...
                if current_ndim >= 3 and "z" in axis_datasets:
                    ...
                    try:
                        ...
                        warnings.warn(
                            f"OverflowError setting label/scale for Z dim (index {z_dim_index}): {e_ovfl}. Skipping dimension link."
                        )
                    except Exception as e:
                        warnings.warn(
                            f"Error setting label/scale for Z dim (index {z_dim_index}): {type(e).__name__} - {e}. Skipping dimension link."
                        )
        ...
        warnings.warn(
            f"An unexpected error occurred while writing HDF5 file {h5_file}: {type(e).__name__} - {e}"
        )
        import traceback
