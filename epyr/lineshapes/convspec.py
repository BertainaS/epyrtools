...
def convspec(spectrum, step_size, width, derivative=0, alpha=1.0, phase=0.0):
    ...
    spectrum = np.asarray(spectrum, dtype=complex if np.iscomplexobj(spectrum) else float)
...
def _expand_parameter(param, ndim):
    ...
    if param.ndim == 0:
        ...
        raise ValueError(f"Parameter length {len(param)} doesn't match spectrum dimensions {ndim}")
...
def _convolve_spectrum(spectrum, step_size, width, derivative, alpha, phase):
    ...
    if alpha[0] == 1.0:
        # Pure Gaussian
        kernel = gaussian(x_kernel, 0, width[0], derivative=int(derivative[0]), phase=phase[0])
    elif alpha[0] == 0.0:
        # Pure Lorentzian
        kernel = lorentzian(x_kernel, 0, width[0], derivative=int(derivative[0]), phase=phase[0])
    else:
        # Mixed (pseudo-Voigt)
        gauss_part = gaussian(x_kernel, 0, width[0], derivative=int(derivative[0]), phase=phase[0])
        lorentz_part = lorentzian(x_kernel, 0, width[0], derivative=int(derivative[0]), phase=phase[0])
...
def demo():
    ...
    try:
        ...
        gauss = ndimage.gaussian_filter1d(stick, sigma=4.0/0.1/(2*np.sqrt(2*np.log(2))))
        ax.plot(x, stick, 'k-', linewidth=3, alpha=0.7, label='Stick spectrum')
    ...
    for i, width in enumerate(widths):
        try:
            ...
            ax.plot(x, convolved, color=colors[i], linewidth=2.5, 
                    label=f'FWHM = {width}')
        except:
            # Fallback
            from scipy import ndimage
    ...
    gauss_theory = np.exp(-x_theory**2 / (2*sigma**2)) / (sigma * np.sqrt(2*np.pi))
    ...
    ax.plot(x_theory, lorentz_theory, color=colors[1], linewidth=2.5, label='Lorentzian')
    ax.plot(x_theory, voigt_theory, color=colors[2], linewidth=2.5, label='Pseudo-Voigt')
    ...
    # Show stick spectrum
    ax.stem(x[epr_stick > 0], epr_stick[epr_stick > 0], 
            linefmt='gray', markerfmt='ko', basefmt=' ', label='Stick spectrum')
