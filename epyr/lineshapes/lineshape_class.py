...
class Lineshape:
    ...
    def __init__(self, 
        ...
        if shape_type not in self.SUPPORTED_SHAPES:
            raise ValueError(f"shape_type must be one of {list(self.SUPPORTED_SHAPES.keys())}")
    ...
    def both_components(self, x: np.ndarray, center: float) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        if hasattr(self._func, 'return_both'):
            ...
            if self.shape_type in ['gaussian', 'lorentzian', 'voigt']:
                return self._func(x, center, self.width,
