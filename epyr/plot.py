...
class EPRPlotConfig:
    ...
    DEFAULT_FIGSIZE_WATERFALL = (6, 4)
    DEFAULT_FIGSIZE_2D = (6, 4)
...
def _validate_data_shape(data_2d: np.ndarray, axis1: np.ndarray, axis2: np.ndarray, 
                        axis1_name: str = "axis1", axis2_name: str = "axis2") -> None:
    """
...
def _get_colormap(cmap_name: str):
    """
    Get colormap using the appropriate method for matplotlib version.
...
def _setup_colorbar_for_lines(fig: plt.Figure, ax: plt.Axes, angle_axis: np.ndarray, 
                             angle_unit: str, cmap_name: str) -> None:
    """
...
def plot_angular_sweep_waterfall(
    ...
    """
    # Validate input data
    _validate_data_shape(spectral_data_2d, angle_axis, field_axis, "angle_axis", "field_axis")
    ...
    data_min, data_max, vertical_offset = _calculate_offset_parameters(spectral_data_2d, offset_factor)
...
def plot_2d_spectral_map(
    ...
    # Create figure if needed
    if ax is None:
        fig, ax = plt.subplots(figsize=EPRPlotConfig.DEFAULT_FIGSIZE_2D)
    else:
        fig = ax.get_figure()
    ...
    # Create mesh plot
    im = ax.pcolormesh(
        x_axis, y_axis, plot_data,
        cmap=cmap_name, shading=shading,
        vmin=vmin, vmax=vmax
    )
...
def plot_epr_comparison(
    ...
    for label, spectrum in spectra_dict.items():
        if len(spectrum) != len(field_axis):
            raise ValueError(f"Spectrum '{label}' length {len(spectrum)} "
...
def generate_example_data():
    ...
    for i, temp in enumerate(temp_axis):
        ...
        center = 3400
        signal = intensity_factor * 80 * linewidth**2 / ((field_axis - center)**2 + linewidth**2)
    ...
    single_spectra['Room Temperature'] = rt_signal + np.random.normal(0, 1, len(field_axis))
    
    # Low temperature spectrum
    lt_signal = 80 * 12**2 / ((field_axis - 3405)**2 + 12**2)
    single_spectra['Low Temperature'] = lt_signal + np.random.normal(0, 0.5, len(field_axis))
    
    # Derivative spectrum
    derivative_signal = np.gradient(rt_signal)
    single_spectra['Derivative'] = derivative_signal + np.random.normal(0, 0.5, len(field_axis))
...
if __name__ == "__main__":
    ...
    try:
        user_input = input("Would you like to run the examples? (y/n): ").lower().strip()
        if user_input in ['y', 'yes', '1', 'true']:
            run_examples()
        else:
            print("Examples skipped. Import this module to use the plotting functions.")
            print("\nExample usage:")
            print("from epr_plotting import plot_angular_sweep_waterfall, generate_example_data")
            print("data = generate_example_data()")
            print("fig, ax = plot_angular_sweep_waterfall(data['field_axis'], data['angle_axis'], data['angular_sweep_data'])")
